/* ParticleScroll — grain-resolves-into-text reveal.
 *
 * Vendored from sublevel.studio (ThreeUI), canonical source
 * public/landing-pages/sublevel-studio.html rev 91db5c1bb779, the inline
 * "Canvas UI Particle Scroll" IIFE. That block is itself described upstream as a
 * "vanilla port of the standalone ParticleScroll.tsx" (the .tsx is not published).
 *
 * VERBATIM from source: the two WebGL2 shader programs, `createParticleScroll`
 * (row-progress model, scroll smoothing, lag, intro gate, context-loss guard),
 * and `rasterise`. Raw WebGL2 — no Three.js.
 *
 * ADAPTED for this project (both changes flagged inline):
 *  1. paintElement — skips `.sr-only` subtrees (RevealText renders a
 *     screen-reader copy that would otherwise ghost-paint), and paintText now
 *     handles soft-wrapped tokens (long hyphen-joined project titles) instead of
 *     drawing the whole word at its first line box.
 *  2. mountParticleScroll — accepts a selector, guards init in try/catch, and
 *     its teardown now also removes the window/ResizeObserver/img listeners it
 *     adds and clears `ps-hidden`/`ps-in` off every tracked element, so a
 *     re-mount (audience toggle) can't leave a heading stuck hidden.
 *
 * The ember tint (DEFAULTS.tintA/tintB) is left as authored but always
 * overridden at the call site with this site's accent colour.
 */

/* once a row is this far into the band it is committed and the floor speed takes over */
const COMMIT = 0.4;
export const DEFAULTS = { point: 0.68, band: 260, density: 2, size: 1.25, spread: 220, gravity: 0.35, drift: 0.7, swirl: 60, stagger: 0.55, fade: 0.85, settle: 0.7, hold: 1.15, smoothing: 0.26, tintA: [1.0, 0.3, 0.0], tintB: [1.0, 0.82, 0.2] };
const HASH = `
float hash (vec2 p) {
...
export function createParticleScroll(elements, options = {}) {
...
function paintElement(root, ctx) {
  /* ADAPTED: reject `.sr-only` subtrees — RevealText renders a screen-reader
     copy of the title that is 1px-clipped, not display:none, so it would
     otherwise be painted on top of the visible words. */
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, {
    acceptNode: (n) => (n.nodeType === 1 && n.classList && n.classList.contains('sr-only'))
  ...
  const paintText = (tn) => { const text = tn.nodeValue; if (!text.trim()) return; const parent = tn.parentElement; const cs = getComputedStyle(parent); ctx.font = `${cs.fontStyle} ${cs.fontWeight} ${cs.fontSize} ${cs.fontFamily}`; ctx.fillStyle = cs.color; ctx.textBaseline = 'alphabetic'; try { ctx.letterSpacing = cs.letterSpacing === 'normal' ? '0px' : cs.letterSpacing; } catch (e) {}
    ...
    const baselineIn = (r) => r.top + (r.height - (asc + desc)) / 2 + asc;
    while ((m = re.exec(text))) {
      ...
      if (rects.length === 1) {
        const r = rects[0]; if (r.width === 0) continue;
        ctx.fillText(m[0], r.left, baselineIn(r));
        continue;
      }
      /* ADAPTED: a soft-wrapped token (long hyphen-joined project titles under
         `overflow-wrap:anywhere`) reports one client rect per line. Walk it
         char-by-char and flush each line's run at that line's own left edge,
         instead of drawing the whole word at rects[0]. */
      const word = m[0];
      let run = '', runRect = null;
      for (let c = 0; c < word.length; c++) {
        range.setStart(tn, m.index + c); range.setEnd(tn, m.index + c + 1);
        const cr = range.getClientRects()[0];
        if (!cr) { run += word[c]; continue; }
        if (runRect && Math.abs(cr.top - runRect.top) > 1) { ctx.fillText(run, runRect.left, baselineIn(runRect)); run = ''; runRect = null; }
        if (!runRect) runRect = cr;
        run += word[c];
      }
      if (run && runRect) ctx.fillText(run, runRect.left, baselineIn(runRect));
    }
  };
...
/* ADAPTED: takes a selector, guards init so a WebGL2 failure is silent, and the
   returned handle's destroy() also unwinds the window/observer/img listeners and
   any lingering `ps-hidden`/`ps-in` classes — the upstream mount leaked those,
   which is fine for a page that never unmounts but not for a re-mount here. */
export function mountParticleScroll(selectorList, options = {}) {
  ...
  imgs.forEach(i => i.addEventListener('load', repaint));
  const bodyObserver = new ResizeObserver(syncUnits); bodyObserver.observe(document.body);
  return {
    repaint,
    destroy() {
      instance.destroy();
      ...
      output.remove();
      for (const t of tracked) t.el.classList.remove('ps-hidden', 'ps-in');
    },
  };
}
